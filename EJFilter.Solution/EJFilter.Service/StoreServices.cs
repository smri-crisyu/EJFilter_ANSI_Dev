...
{
    public interface IStoreServices : IService<MasterStore>
    {
    ...
    {
        private readonly IRepository<MasterStore> repository;
        private IResult result;

        private readonly string className = MethodBase.GetCurrentMethod().DeclaringType.Name;

        public StoreServices()
         : this(new EFGenericRepository<MasterStore>(new EJFilterContextDB()))
        {

        }

        public StoreServices(IRepository<MasterStore> inRepo)
        {
            repository = inRepo;
            result = new Result();
        }

        public IResult Add(MasterStore entity)
        {
            if (entity == null)
            {
        ...
        public List<MasterStore> GetAll()
        {
            return repository.GetAll().ToList();
        }

        public MasterStore GetById(int id)
        {
            return repository.GetById(id);
        }

        public IResult Remove(MasterStore entity)
        {
            if (entity == null)
            {
                result.Success = false;
        ...
        public IResult Update(MasterStore entity)
        {
            if (entity == null)
            {
                result.Success = false;
