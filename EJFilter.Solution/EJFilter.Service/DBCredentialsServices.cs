...
{
    public interface IDBCredentialsServices : IService<DBCredentials>
    {
       

    }

    public class DBCredentialsServices : IDBCredentialsServices
    {
        private readonly IRepository<DBCredentials> repository;
        private IResult result;

        private readonly string className = MethodBase.GetCurrentMethod().DeclaringType.Name;

        public DBCredentialsServices()
         : this(new EFGenericRepository<DBCredentials>(new EJFilterContextDB()))
        {

        }

        public DBCredentialsServices(IRepository<DBCredentials> inRepo)
        {
            repository = inRepo;
            result = new Result();
        }

        public IResult Add(DBCredentials entity)
        {
            if (entity == null)
            {
        ...
        public DBCredentials GetById(int id)
        {
            return repository.GetById(id);
        }

        public IResult Remove(DBCredentials entity)
        {
            if (entity == null)
            {
                result.Success = false;
        ...
        public IResult Update(DBCredentials entity)
        {
            if (entity == null)
            {
                result.Success = false;
