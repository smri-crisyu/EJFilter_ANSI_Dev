using EJFilter.Models.Entity;
using EJFilter.Models.Enum;
using EJFilter.Models.Interface;
using EJFilter.Models;
using EJFilter.Repository;
using System;
using System.Collections.Generic;
using System.Linq;
using System.Reflection;
using System.Text;
using System.Threading.Tasks;

namespace EJFilter.Service
{
    public interface IHeaderFooterFormatServices : IService<HeaderFooterTPFormat>
    {
        HeaderFooterTPFormat GetBranchesByCode(int branchCode);
        List<HeaderFooterTPFormat> GetBranchesByStoreCode(int storeCode);
        HeaderFooterTPFormat GetBranchByBranchCodeAndStoreCode(int branchCode, int storeCode);

    }

    public class HeaderFooterFormatServices : IHeaderFooterFormatServices
    {
        private readonly IRepository<HeaderFooterTPFormat> repository;
        private IResult result;

        private readonly string className = MethodBase.GetCurrentMethod().DeclaringType.Name;

        public HeaderFooterFormatServices()
         : this(new EFGenericRepository<HeaderFooterTPFormat>(new EJFilterContextDB()))
        {

        }
        public HeaderFooterFormatServices(IRepository<HeaderFooterTPFormat> inRepo)
        {
            repository = inRepo;
            result = new Result();
        }

        public IResult Add(HeaderFooterTPFormat entity)
        {
            if (entity == null)
            {
                result.Success = false;
                result.ErrorCode = (int)ErrorCode.ObjectCannotBeNull;
                result.ReturnMessage = BaseVariable.ErrorDescription((int)ErrorCode.ObjectCannotBeNull);
                return result;
            }

            repository.Create(entity);

            if (repository.SaveChanges() == (int)BaseEnum.SuccessCode.ProcessSuccess)
            {
                result.Success = true;
            }
            else
            {
                result.ReturnMessage = BaseVariable.ErrorDescription((int)ErrorCode.AddDataFailed);
                result.ErrorCode = (int)ErrorCode.AddDataFailed;
                result.Environment = className + "-" + this.GetType().FullName;
            }


            return result;
        }

        public List<HeaderFooterTPFormat> GetAll()
        {
            return repository.GetAll().ToList();
        }

        public HeaderFooterTPFormat GetById(int id)
        {
            return repository.GetById(id);
        }

        public IResult Remove(HeaderFooterTPFormat entity)
        {
            if (entity == null)
            {
                result.Success = false;
                result.ErrorCode = (int)ErrorCode.ObjectCannotBeNull;
                result.ReturnMessage = BaseVariable.ErrorDescription((int)ErrorCode.ObjectCannotBeNull);
                return result;
            }

            repository.Delete(entity);

            if (repository.SaveChanges() == (int)BaseEnum.SuccessCode.ProcessSuccess)
            {
                result.Success = true;
            }
            else
            {
                result.ReturnMessage = BaseVariable.ErrorDescription((int)ErrorCode.DeleteDataFailed);
                result.ErrorCode = (int)ErrorCode.DeleteDataFailed;
                result.Environment = className + "-" + this.GetType().FullName;
            }
            return result;
        }

        public IResult Update(HeaderFooterTPFormat entity)
        {
            if (entity == null)
            {
                result.Success = false;
                result.ErrorCode = (int)ErrorCode.ObjectCannotBeNull;
                result.ReturnMessage = BaseVariable.ErrorDescription((int)ErrorCode.ObjectCannotBeNull);
                return result;
            }

            repository.Update(entity);

            if (repository.SaveChanges() == (int)BaseEnum.SuccessCode.ProcessSuccess)
            {
                result.Success = true;
            }
            else
            {
                result.ReturnMessage = BaseVariable.ErrorDescription((int)ErrorCode.UpdateDataFailed);
                result.ErrorCode = (int)ErrorCode.UpdateDataFailed;
                result.Environment = className + "-" + this.GetType().FullName;
            }



            return result;
        }

        public HeaderFooterTPFormat GetBranchesByCode(int branchCode)
        {
            return repository.Read(x => x.BranchCode == branchCode);
        }

        public List<HeaderFooterTPFormat> GetBranchesByStoreCode(int storeCode)
        {
            return repository.GetWhere(x => x.StoreCode == storeCode).ToList();
        }

        public HeaderFooterTPFormat GetBranchByBranchCodeAndStoreCode(int branchCode, int storeCode)
        {
            return repository.Read(x => x.BranchCode == branchCode && x.StoreCode == storeCode);
        }
    }
}
